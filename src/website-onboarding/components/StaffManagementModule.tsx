...
import { generateBio } from '../lib/aiAssist';
import { 
  Users, 
  UserCheck, 
  Clock, 
...
export default function StaffManagementModule({ data, setData, onSave, onBackToWizard }: Props) {
  ...
  const handleSubmitForm = (e: FormEvent) => {
    ...
    const newOrUpdatedMember: TeamMember = {
      ...
      bio: formBio.trim(),
      // Never seed a rating — it can only come from real customer reviews once
      // the website is live. Keep an existing one when editing.
      rating: editingMemberId
        ? data.team.find(m => m.id === editingMemberId)?.rating
        : undefined
    };
                      ...
                      {filteredStaff.map(member => {
                        ...
                        return (
                          <tr key={member.id} className="hover:bg-gray-50/80 transition-colors group">
                            ...
                            {/* Rating — real ratings only exist after the site is live and
                                customers review; never invent one here. */}
                            <td className="py-4 px-6">
                              <span className="text-xs font-semibold text-gray-400">
                                {typeof member.rating === 'number' ? member.rating.toFixed(1) : 'No reviews yet'}
                              </span>
                            </td>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-2xs space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Set the commission rate for each staff member. This is saved as part of the shop
                  setup — it is not a payout instruction.
                </p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-2.5">
                ...
                <p className="text-xs text-amber-900 leading-relaxed">
                  No revenue or payout figures are shown here. Real earnings appear in the Shop
                  Owner&apos;s dashboard once the website is live and bookings are completed. Your own
                  Growth Partner commission is tracked separately in Earnings.
                </p>
              </div>
