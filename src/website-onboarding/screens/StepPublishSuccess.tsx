...
import { improveText } from '../lib/aiAssist';
import { 
  CheckCircle2, 
  Globe, 
  Copy, 
...
export default function StepPublishSuccess({ data, setData, onNext, onSave }: Props) {
  ...
  const url = data.publishedUrl || `https://nexora.site/${data.websiteSlug || 'your-website'}`;
  ...
  const handleWhatsAppShare = () => {
    const text = `🎉 My salon website is on the way! It will be live at: ${url} — built with Nexora ✨`;
  ...
  const handleTuneContent = async (simplify: boolean = false) => {
    ...
    try {
      const rewritten = await improveText({
        text: originalText,
        field: apiFieldKey,
        tone: tuningTone,
        keywords: '',
      ...
      <header className="bg-white border-b border-gray-200 h-16 w-full flex justify-between items-center px-6 md:px-10 z-10 sticky top-0 shrink-0 shadow-sm">
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="flex items-center gap-1.5 text-[#ac0053]">
            <span className="text-xs font-bold">Sent for Owner Approval</span>
            ...
            <div className="space-y-5">
              <div className="flex flex-col md:flex-row items-center gap-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="flex justify-center md:justify-start items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Awaiting owner review</span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full space-y-2">
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider text-left">
                  Your Web Address — live once the owner approves
                </label>
              ...
              {/*
                Existing Growth Partner lifecycle, shown (not re-implemented):
                proposal 'submitted' -> Shop Owner reviews & publishes -> bookings
                at the attributed shop accrue commission in the existing ledger
                (growth_partner_commissions, read-only in this app).
              */}
              <div className="bg-emerald-50/70 border border-emerald-200 rounded-xl p-3 text-left">
                <p className="text-[11px] font-semibold text-emerald-900 leading-relaxed">
                  Next: the Shop Owner reviews and publishes the website. Your attribution for
                  this shop is unchanged — once it is live, bookings keep earning your usual
                  commission, tracked in Earnings.
                </p>
              </div>
            ...
            >
              ...
              <div className="text-center space-y-4">
                <div className="space-y-1 pt-2">
                  ...
                  <p className="text-xs text-gray-500">Points to the reserved address. It opens the website once the Shop Owner publishes it.</p>
                </div>
