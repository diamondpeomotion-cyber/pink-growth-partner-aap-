...
export default function StepPublishSetup({ data, setData, onNext, onPrev, onSave }: Props) {
  const [slug, setSlug] = useState<string>(data.websiteSlug || slugify(data.salonName));
  const [mode, setMode] = useState<'desktop' | 'mobile'>('desktop');
  const [publishing, setPublishing] = useState(false);

  // The initial slug is already derived from the salon name in useState above,
  // and the effect below immediately writes it back to data.websiteSlug, so the
  // old "sync slug from name" effect could never fire a second time. Removed to
  // avoid a setState-inside-effect cascade.
  ...
  const handlePublish = () => {
    ...
    if (onSave) onSave();
    setTimeout(() => {
      // Submitting hands the website to the Shop Owner for review. Publishing
      // stays an owner/server-side action (salon_setup_proposals.published_at),
      // so the partner-side payload must not claim 'published'.
      setData(prev => ({ ...prev, publishState: 'submitted', publishedUrl: fullUrl, lastCompletedStep: 14 }));
      if (onSave) onSave();
      setPublishing(false);
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-2xs flex flex-col gap-5">
            ...
            {!allRequiredDone && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700 flex gap-2">
                ...
                Please complete all required fields above before submitting for approval.
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="h-[76px] bg-white border-t border-gray-200 flex items-center justify-between px-6 shrink-0 z-10 shadow-xs">
        ...
        >
          {publishing ? (
            ...
            <>
              <Globe className="w-4 h-4" /> Submit for Approval
            </>
          )}
        </button>
      </footer>
