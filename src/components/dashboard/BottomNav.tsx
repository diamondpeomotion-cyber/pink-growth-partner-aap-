...
export default function BottomNav({ onNavigate, currentPage }: { onNavigate?: (page: string) => void, currentPage?: string }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/80 backdrop-blur-md border-t border-gray-100 shadow-[0px_-4px_20px_rgba(0,0,0,0.03)]">
      <div className="max-w-screen-xl mx-auto w-full flex justify-around items-center h-[calc(5rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] px-4 sm:px-6 md:px-8">
        {items.map((item, idx) => {
