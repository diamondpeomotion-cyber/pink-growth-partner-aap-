...
export default function ShopSummary({
  ...
  const cards = [
    { title: 'Total attributed', value: String(total), desc: 'Shops on your attribution ledger' },
    { title: 'Active', value: String(active), desc: 'status = active' },
    { title: 'Other statuses', value: String(Math.max(0, total - active)), desc: 'Not yet active' },
  ];
  ...
  return (
    <section className="w-full space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {cards.map((card) => (
          <div key={card.title} className="bg-white rounded-[18px] p-4 border border-[#e4e2e1]/40 shadow-[0px_4px_20px_rgba(0,0,0,0.03)] flex flex-col">
